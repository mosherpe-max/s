...
function modePathFor(mode) {
    if (mode === 'Clubhouse')
        return 'clubhouse';
    if (mode === 'Lane Delivery')
        return 'laneside';
    return 'bevcart';
}
 ...
 */
async function pushToActiveStaff(sellerId, mode, payload) {
    ...
    if (staffSnap.empty)
        return;
    const deviceIds = Array.from(new Set(staffSnap.docs.map(d => d.data()?.currentDeviceId).filter((id) => !!id)));
...
export const notifyStaffOnNewOrder = onDocumentWritten({
    ...
    if (!order?.sellerId || !order?.menuType)
        return;
    try {
...
export const checkLateOrders = onSchedule({
    ...
    for (const orderDoc of activeSnap.docs) {
        ...
        if (order.lateAlertSentAt || !order.createdAt || !order.sellerId || !order.menuType)
            continue;
        ...
        if (elapsedMinutes < thresholds.maxOrderProcessingMinutes)
            continue;
        try {
            await pushToActiveStaff(order.sellerId, order.menuType, {
