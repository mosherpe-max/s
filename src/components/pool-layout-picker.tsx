 ...
 * If no image is provided, it falls back to a default placeholder layout.
 */
export function PoolLayoutPicker({ value, onChange, mapUrl }: PoolLayoutPickerProps) {
  ...
  return (
    <div className="space-y-4">
      <div className="relative w-full aspect-[3/4] bg-muted rounded-[2rem] overflow-hidden border-4 border-white shadow-2xl group">
        ...
        {/* Darker overlay for better zone visibility */}
              ...
              >
                <div className="flex flex-col items-center">
                  {isSelected ? (
                    ...
                    <span className="font-headline font-black text-2xl drop-shadow-md">
                      {zone.label}
                    </span>
                  )}
                </div>
        ...
        {value ? (
          ...
          <div className="flex flex-col items-center gap-1.5 px-4 text-center">
            <p className="text-[11px] font-black uppercase tracking-[0.2em] text-primary flex items-center gap-2">
              <Info className="h-3.5 w-3.5" /> Tap a Numbered Zone
            </p>
            <p className="text-[9px] font-bold text-muted-foreground uppercase tracking-widest leading-relaxed">
              Select the area closest to where you are sitting so we can find you.
            </p>
          </div>
        )}
      </div>
