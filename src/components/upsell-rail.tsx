...
export function UpsellRail({ upsellItemIds, menuItems, orderItems, onAdd }: UpsellRailProps) {
  ...
  // Once collapsed, drop out of the layout entirely - otherwise the
  // parent's flex gap spacing would leave a small residual gap even
  // though this section is visually zero-height.
  useEffect(() => {
    if (!isHidden) return;
  ...
  if (upsellItemIds.length === 0 || removed) return null;
  ...
  return (
    // grid-template-rows 1fr->0fr is a JS-free way to animate to/from auto
    // height - collapsing this smoothly (rather than unmounting instantly)
    // is what makes the pricing card below visibly slide up into the freed
    // space on "No Thanks" instead of just snapping into place.
    <div
      className={cn(
    ...
    >
      <div className="overflow-hidden min-h-0">
        <div className="bg-white rounded-2xl border-2 border-slate-100 shadow-sm px-4 pt-4 pb-4">
