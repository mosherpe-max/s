...
export default function SellerAdminPage({ params }: { params: Promise<{ sellerId: string }> }) {
  ...
  const stats = useMemo(() => {
    ...
    return {
      ...
      fees: fees.toFixed(2),
      active: filteredOrders.filter(o => o.status !== 'Delivered' && o.status !== 'Cancelled').length,
      volume: today.length,
  ...
  const analyticsData = useMemo(() => {
    ...
    if (analyticsRange === 'Today') {
      ...
      const intervals = Array.from({ length: now.getMonth() + 1 }, (_, i) => addMonths(start, i));
  ...
  const mappedBuyers = useMemo(() => {
    if (!orders) return [];
    return orders.filter(o => o.status !== 'Delivered' && o.status !== 'Cancelled').map(o => ({
      id: o.id, name: o.customerName, location: o.deliveryLocation, colorClass: o.menuType === 'Beverage Cart' ? "bg-red-600" : "bg-indigo-600"
    }));
  }, [orders]);
  ...
  if (isSellerLoading || isRoleLoading || isUserLoading || isVenueLoading) {
    return (
  ...
  const compositionCategories = [...categories].sort((a, b) => (a === 'Featured' ? -1 : b === 'Featured' ? 1 : 0));
            ...
            <div className="max-w-6xl mx-auto space-y-10 pb-24">
                ...
                <div className="space-y-6 sm:space-y-8 animate-in fade-in duration-500">
                  ...
                  <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                    <Card className="lg:col-span-2 border-2 shadow-sm overflow-hidden">
                      <CardHeader className="bg-slate-50/50 border-b flex flex-row items-center justify-between py-4"><div className="space-y-0.5"><CardTitle className="text-xs font-black uppercase tracking-widest">Today's Pulse</CardTitle><CardDescription className="text-[9px] font-bold uppercase">Hourly Sales & Deliveries</CardDescription></div><Badge variant="outline" className="text-[9px] font-black uppercase border-2 text-indigo-600 border-indigo-100">{dashboardFilter}</Badge></CardHeader>
                      <CardContent className="pt-8 h-[300px] sm:h-[350px]">
                        <ResponsiveContainer width="100%" height="100%">
                          <BarChart data={analyticsData.chartData}><CartesianGrid strokeDasharray="3 3" vertical={false} stroke="#E2E8F0" /><XAxis dataKey="time" axisLine={false} tickLine={false} fontSize={10} fontWeight="bold" tick={{ fill: '#64748B' }} /><YAxis axisLine={false} tickLine={false} fontSize={10} fontWeight="bold" tick={{ fill: '#64748B' }} /><ChartTooltip cursor={{ fill: 'transparent' }} contentStyle={{ borderRadius: '16px', border: '2px solid #E2E8F0', padding: '12px' }} /><Legend verticalAlign="top" align="right" iconType="circle" wrapperStyle={{ fontSize: '10px', textTransform: 'uppercase', fontWeight: 'bold' }} />{seller?.menuTypes?.map(mode => (<Bar key={mode} stackId="a" dataKey={mode} fill={MODE_COLORS[mode] || '#64748B'} radius={[0, 0, 0, 0]} barSize={20} />))}</BarChart>
                        </ResponsiveContainer>
                      </CardContent>
                    </Card>
                    <Card className="border-2 shadow-sm"><CardHeader className="bg-slate-50/50 border-b"><CardTitle className="text-xs font-black uppercase tracking-widest">Active Channels</CardTitle></CardHeader><CardContent className="p-4 space-y-3">
              ...
              {activeNav === 'service' && (
                <div className="space-y-12 animate-in fade-in duration-500">
                  <div className="space-y-6"><div className="flex items-center gap-3"><div className="bg-primary/10 p-2 rounded-xl text-primary"><Zap className="h-5 w-5" /></div><h3 className="font-headline font-black text-lg text-[#213147] uppercase">Channel Status</h3></div><div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-3">
                    {['Beverage Cart', 'Clubhouse', 'Lane Delivery', 'Take Out'].map(mode => {
                      const isActive = (mode === 'Beverage Cart' && seller?.bevcartActive) || (mode === 'Clubhouse' && seller?.clubhouseActive) || (mode === 'Lane Delivery' && seller?.lanedeliveryActive) || (mode === 'Take Out' && seller?.takeoutActive);
                      if (!seller?.menuTypes?.includes(mode)) return null;
                      return (<Card key={mode} className={cn("border-2 transition-all cursor-pointer group", isActive ? "border-primary bg-primary/5 shadow-md" : "border-slate-100 hover:border-slate-200")} onClick={() => handleToggleMode(mode, !!isActive)}><CardContent className="p-3 flex flex-col items-center text-center gap-2"><div className={cn("p-3 rounded-xl transition-all group-hover:scale-110", isActive ? "bg-primary text-white" : "bg-slate-100 text-slate-400")}><Zap className="h-4 w-4" /></div><div className="space-y-1"><p className="font-black uppercase text-[10px] text-[#213147] leading-tight truncate w-full">{mode}</p><Badge variant="outline" className={cn("text-[8px] font-black uppercase h-4 px-2", isActive ? "bg-green-50 text-green-600 border-green-100" : "bg-slate-50 text-slate-400 border-slate-200")}>{isActive ? 'ON' : 'OFF'}</Badge></div></CardContent></Card>);
                    })}
                  </div></div>
              ...
              {activeNav === 'payments' && (
                <div className="space-y-8 animate-in fade-in duration-500"><div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6"><Card className="bg-[#213147] text-white border-0 shadow-xl overflow-hidden relative"><div className="absolute top-0 right-0 p-8 opacity-10"><DollarSign className="h-24 w-24" /></div><CardHeader className="pb-2 pt-6 px-6"><CardDescription className="text-white/60 text-[9px] font-black uppercase tracking-widest">Available Revenue</CardDescription></CardHeader><CardContent className="px-6 pb-8"><div className="text-3xl sm:text-4xl font-black font-headline tracking-tighter">${stats?.revenue}</div></CardContent></Card><Card className="border-2 border-indigo-100 bg-indigo-50/30"><CardHeader className="pb-2 pt-6 px-6"><CardDescription className="text-indigo-600/60 text-[9px] font-black uppercase tracking-widest">Platform Fees</CardDescription></CardHeader><CardContent className="px-6 pb-8"><div className="text-3xl sm:text-4xl font-black font-headline tracking-tighter text-indigo-700">${stats?.fees}</div></CardContent></Card><Card className="border-2 border-amber-100 bg-amber-50/30"><CardHeader className="pb-2 pt-6 px-6"><CardDescription className="text-amber-600/60 text-[9px] font-black uppercase tracking-widest">Venue Commission</CardDescription></CardHeader><CardContent className="px-6 pb-8"><div className="text-3xl sm:text-4xl font-black font-headline tracking-tighter text-amber-700">100%</div></CardContent></Card></div></div>
              )}

              {activeNav === 'stripe' && (
                <div className="space-y-8 animate-in fade-in duration-500"><h3 className="font-headline font-black text-lg text-[#213147] uppercase">Stripe Settings</h3><Card className="border-2 shadow-sm max-w-2xl"><CardHeader className="bg-slate-50/50 border-b flex flex-row items-center justify-between"><div className="space-y-1"><CardTitle className="text-xs font-black uppercase tracking-widest">Stripe Engine</CardTitle><CardDescription className="text-[9px] uppercase font-bold text-muted-foreground">Revenue disbursement configuration</CardDescription></div><Badge className="bg-indigo-600 uppercase text-[8px] font-black">PCI-DSS Secure</Badge></CardHeader><CardContent className="p-4 sm:p-6 space-y-4"><div className="flex flex-col sm:flex-row items-start sm:items-center justify-between p-4 bg-white rounded-2xl border-2 border-indigo-100 gap-4"><div className="flex items-center gap-4"><div className="p-2 bg-indigo-50 rounded-xl text-indigo-600"><ShieldCheck className="h-6 w-6" /></div><div><p className="text-[10px] font-black uppercase text-[#213147]">Status: {venueData?.payoutsEnabled ? 'Live' : 'Onboarding'}</p><p className="text-[8px] font-bold text-indigo-600 uppercase">Payouts Settlement: Direct to Bank</p></div></div><Button variant="outline" size="sm" onClick={handleVerifyStripe} disabled={isVerifyingStripe} className="w-full sm:w-auto text-[9px] font-black uppercase border-2">{isVerifyingStripe ? <Loader2 className="animate-spin" /> : "Run Diagnostic"}</Button></div>{verificationResult && <div className="p-3 bg-slate-50 rounded-xl border-2 border-dashed text-center"><p className="text-[8px] font-black uppercase text-indigo-600">{verificationResult.businessName}: {verificationResult.status}</p></div>}</CardContent></Card></div>
              )}

              {activeNav === 'analytics' && (
                <div className="space-y-12 animate-in fade-in duration-500">
                  <div className="grid grid-cols-1 lg:grid-cols-2 gap-8"><Card className="border-2 shadow-sm"><CardHeader className="bg-slate-50/50 border-b flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4"><div className="space-y-0.5"><CardTitle className="text-xs font-black uppercase tracking-widest text-[#213147]">Revenue Distribution</CardTitle><CardDescription className="text-[8px] font-bold uppercase">Stacked performance by mode</CardDescription></div><div className="flex bg-slate-100 p-0.5 rounded-lg border-2">{['Today', 'MTD', 'YTD'].map((r) => (<button key={r} onClick={() => setAnalyticsRange(r as any)} className={cn("px-3 py-1 text-[8px] font-black uppercase tracking-tighter rounded-md transition-all", analyticsRange === r ? "bg-white text-[#213147] shadow-sm" : "text-slate-400 hover:text-slate-600")}>{r}</button>))}</div></CardHeader><CardContent className="pt-10 h-[300px]"><ResponsiveContainer width="100%" height="100%"><BarChart data={analyticsData.chartData}><CartesianGrid strokeDasharray="3 3" vertical={false} stroke="#E2E8F0" /><XAxis dataKey="time" axisLine={false} tickLine={false} fontSize={10} fontWeight="bold" /><YAxis axisLine={false} tickLine={false} fontSize={10} fontWeight="bold" /><ChartTooltip cursor={{ fill: 'transparent' }} contentStyle={{ fontSize: '10px', borderRadius: '12px', border: '2px solid #E2E8F0' }} /><Legend verticalAlign="top" align="right" iconType="circle" wrapperStyle={{ fontSize: '9px', fontWeight: 'bold', textTransform: 'uppercase' }} />{seller?.menuTypes?.map(mode => (<Bar key={mode} dataKey={mode} stackId="a" fill={MODE_COLORS[mode] || '#64748B'} radius={[0, 0, 0, 0]} label={(props: any) => { const { x, y, width, height, value, index } = props; if (value <= 0 || height < 15) return null; const entry = analyticsData.chartData[index]; let labelText = analyticsRange === 'YTD' ? `$${(entry[`${mode}_count`] > 0 ? (value / entry[`${mode}_count`]).toFixed(0) : '0')}` : (entry[`${mode}_count`] || 0).toString(); return (<text x={x + width / 2} y={y + height / 2} fill="#FFFFFF" textAnchor="middle" dominantBaseline="middle" fontSize={height < 20 ? 7 : 8} fontWeight="900" className="pointer-events-none drop-shadow-sm">{labelText}</text>); }} />))}</BarChart></ResponsiveContainer></CardContent></Card><Card className="border-2 shadow-sm"><CardHeader className="bg-slate-50/50 border-b flex flex-row items-center justify-between py-4"><div className="space-y-0.5"><CardTitle className="text-xs font-black uppercase tracking-widest text-[#213147]">Revenue Share</CardTitle><CardDescription className="text-[8px] font-bold uppercase">Channel Mix & Efficiency</CardDescription></div><Select value={pieMonthFilter} onValueChange={setPieMonthFilter}><SelectTrigger className="w-[140px] h-8 text-[9px] font-black uppercase tracking-widest border-2 bg-white"><SelectValue placeholder="All Time" /></SelectTrigger><SelectContent><SelectItem value="All">All Time</SelectItem>{availableMonths.map(m => <SelectItem key={m} value={m}>{m}</SelectItem>)}</SelectContent></Select></CardHeader><CardContent className="p-0"><div className="h-[240px] pt-4"><ResponsiveContainer width="100%" height="100%"><PieChart><Pie data={pieChartData} innerRadius={50} outerRadius={70} paddingAngle={5} dataKey="value" animationDuration={1000}>{pieChartData.map((entry, index) => (<Cell key={`cell-${index}`} fill={MODE_COLORS[entry.name] || PIE_COLORS[index % PIE_COLORS.length]} />))}</Pie><ChartTooltip formatter={(value: number) => `$${value.toFixed(2)}`} /></PieChart></ResponsiveContainer></div><div className="px-4 pb-6 space-y-2">{pieChartData.map((data, idx) => (<div key={data.name} className="flex items-center justify-between p-2.5 rounded-xl border-2 bg-slate-50/30"><div className="flex items-center gap-2"><div className="w-2 h-2 rounded-full" style={{ backgroundColor: MODE_COLORS[data.name] || PIE_COLORS[idx % PIE_COLORS.length] }} /><span className="text-[10px] font-black uppercase text-[#213147]">{data.name}</span></div><div className="flex items-center gap-6"><div className="text-right"><p className="text-[8px] font-bold text-muted-foreground uppercase leading-none mb-1">Rev / Orders</p><p className="text-[10px] font-black text-[#213147] font-mono">${data.value.toFixed(2)} <span className="text-slate-400">({data.count})</span></p></div><div className="text-right border-l pl-4"><p className="text-[8px] font-bold text-muted-foreground uppercase leading-none mb-1">Avg Ticket</p><p className="text-[10px] font-black text-primary font-mono">${data.avg.toFixed(2)}</p></div></div></div>))}</div></CardContent></Card></div>
                  <div className="space-y-6"><div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 border-b-2 pb-4"><div className="flex items-center gap-3"><div className="bg-primary/10 p-2 rounded-xl text-primary"><FileText className="h-5 w-5" /></div><div><h3 className="font-headline font-black text-xl text-[#213147] uppercase leading-tight">Detailed Sales Audit</h3><p className="text-[10px] font-bold text-muted-foreground uppercase tracking-widest">Transaction level reporting</p></div></div><div className="flex flex-wrap items-center gap-2"><div className="flex items-center bg-white border-2 rounded-xl px-3 h-10 gap-2"><Calendar className="h-3.5 w-3.5 text-muted-foreground" /><input type="date" value={reportStartDate} onChange={(e) => setReportStartDate(e.target.value)} className="text-[10px] font-black uppercase bg-transparent outline-none" /><span className="text-[10px] text-muted-foreground font-bold">TO</span><input type="date" value={reportEndDate} onChange={(e) => setReportEndDate(e.target.value)} className="text-[10px] font-black uppercase bg-transparent outline-none" /></div><Select value={reportModeFilter} onValueChange={setReportModeFilter}><SelectTrigger className="w-[140px] h-10 border-2 font-black uppercase tracking-widest text-[9px] bg-white"><SelectValue placeholder="All Modes" /></SelectTrigger><SelectContent><SelectItem value="All">All Modes</SelectItem>{seller?.menuTypes?.map(m => <SelectItem key={m} value={m}>{m}</SelectItem>)}</SelectContent></Select></div></div><div className="grid grid-cols-1 sm:grid-cols-3 gap-4"><div className="bg-white p-4 rounded-2xl border-2 shadow-sm flex flex-col justify-center"><p className="text-[8px] font-black text-muted-foreground uppercase tracking-[0.2em] mb-1">Audit Volume</p><p className="text-xl font-black font-headline text-[#213147]">{detailedReportStats.volume} <span className="text-[10px] text-muted-foreground uppercase font-bold">Tickets</span></p></div><div className="bg-white p-4 rounded-2xl border-2 shadow-sm flex flex-col justify-center"><p className="text-[8px] font-black text-muted-foreground uppercase tracking-[0.2em] mb-1">Audit Revenue</p><p className="text-xl font-black font-headline text-green-600">${detailedReportStats.revenue.toFixed(2)}</p></div><Button variant="outline" className="h-full border-2 rounded-2xl gap-2 font-black uppercase text-[10px] tracking-widest"><Download className="h-4 w-4" /> Export Audit Log</Button></div><Card className="border-2 shadow-sm overflow-hidden"><div className="overflow-x-auto"><Table><TableHeader className="bg-slate-50 border-b"><TableRow><TableHead className="text-[9px] font-black uppercase tracking-widest">Order ID</TableHead><TableHead className="text-[9px] font-black uppercase tracking-widest">Timestamp</TableHead><TableHead className="text-[9px] font-black uppercase tracking-widest">Patron</TableHead><TableHead className="text-[9px] font-black uppercase tracking-widest">Mode</TableHead><TableHead className="text-[9px] font-black uppercase tracking-widest">Total</TableHead><TableHead className="text-[9px] font-black uppercase tracking-widest text-right">Status</TableHead></TableRow></TableHeader><TableBody>{detailedReportOrders.length === 0 ? (<TableRow><TableCell colSpan={6} className="h-32 text-center text-[10px] font-bold text-muted-foreground uppercase">No transactions found for this audit window.</TableCell></TableRow>) : (detailedReportOrders.map((o) => (<TableRow key={o.id}><TableCell className="font-mono text-[10px] font-black">#{getNumericOrderId(o.id)}</TableCell><TableCell className="text-[10px] font-bold text-slate-500 uppercase">{o.createdAt && typeof o.createdAt.toDate === 'function' ? format(o.createdAt.toDate(), 'MMM d, h:mm a') : 'N/A'}</TableCell><TableCell className="text-[10px] font-black text-[#213147] uppercase truncate max-w-[120px]">{o.customerName}</TableCell><TableCell><Badge variant="secondary" className="text-[8px] font-black uppercase">{o.menuType}</Badge></TableCell><TableCell className="font-mono text-[10px] font-black text-primary">${(o.total || 0).toFixed(2)}</TableCell><TableCell className="text-right"><Badge className={cn("text-[8px] font-black uppercase border-0", o.status === 'Delivered' ? 'bg-green-600' : 'bg-slate-400')}>{o.status}</Badge></TableCell></TableRow>)))}</TableBody></Table></div></Card></div>
                </div>
              )}

              {activeNav === 'settings' && (
                <div className="space-y-10 animate-in fade-in duration-500"><div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4"><h3 className="font-headline font-black text-xl text-[#213147] uppercase">Venue Settings</h3><Button onClick={handleUpdateVenueRegistry} disabled={isProcessingSave} className="w-full sm:w-auto bg-primary hover:bg-primary/90 text-white font-black uppercase tracking-widest px-8 gap-2 shadow-xl h-12">{isProcessingSave ? <Loader2 className="h-4 w-4 animate-spin" /> : <Save className="h-4 w-4" />} Synchronize Terminal</Button></div><div className="grid grid-cols-1 lg:grid-cols-2 gap-8"><Card className="border-2 shadow-sm"><CardHeader className="bg-slate-50/50 border-b flex flex-row items-center gap-3"><Target className="h-4 w-4 text-indigo-600" /><CardTitle className="text-xs font-black uppercase tracking-widest">Venue Profile</CardTitle></CardHeader><CardContent className="p-4 sm:p-6 space-y-4"><div className="grid grid-cols-1 gap-4"><div className="space-y-2"><Label className="text-[10px] font-black uppercase text-muted-foreground">Official Venue Name</Label><Input value={venueName} onChange={e => setVenueName(e.target.value)} className="border-2 font-bold h-11" /></div><div className="space-y-2"><Label className="text-[10px] font-black uppercase text-muted-foreground">Sales Tax Rate (%)</Label><Input type="number" value={venueTaxRate} onChange={e => setVenueTaxRate(Number(e.target.value))} className="border-2 font-bold h-11" /></div></div></CardContent></Card><Card className="border-2 shadow-sm"><CardHeader className="bg-slate-50/50 border-b flex flex-row items-center gap-3"><Timer className="h-4 w-4 text-primary" /><CardTitle className="text-xs font-black uppercase tracking-widest">Delivery Thresholds</CardTitle></CardHeader><CardContent className="p-4 sm:p-6"><p className="text-[9px] font-bold text-muted-foreground uppercase mb-6 leading-relaxed">Define fulfill times in minutes. Alerts will trigger based on these settings.</p><div className="space-y-6">{['Beverage Cart', 'Clubhouse', 'Lane Delivery', 'Take Out'].map(mode => { const thresholds = venueThresholds[mode] || DEFAULT_THRESHOLDS[mode] || { warning: 15, max: 20 }; return (<div key={mode} className="space-y-3 p-4 bg-slate-50 rounded-2xl border-2"><div className="flex items-center justify-between mb-1"><span className="text-[10px] font-black uppercase text-[#213147]">{mode}</span><Badge variant="outline" className="text-[8px] font-bold">Active</Badge></div><div className="grid grid-cols-2 gap-4"><div className="space-y-1.5"><Label className="text-[8px] font-black uppercase text-amber-600">Warn (Min)</Label><Input type="number" min="0" step="1" value={thresholds.warning} onChange={e => handleThresholdChange(mode, 'warning', e.target.value)} className="h-10 border-2 font-bold focus-visible:ring-amber-400" /></div><div className="space-y-1.5"><Label className="text-[8px] font-black uppercase text-red-600">Max (Overdue)</Label><Input type="number" min="0" step="1" value={thresholds.max} onChange={e => handleThresholdChange(mode, 'max', e.target.value)} className="h-10 border-2 font-bold focus-visible:ring-red-600" /></div></div></div>); })}</div></CardContent></Card></div></div>
              )}
