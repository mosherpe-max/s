...
export default function SellerAdminPage({ params }: { params: Promise<{ sellerId: string }> }) {
  ...
  const handleImpersonate = (mode: string) => {
    ...
    toast({ title: `Entering ${mode} View`, description: "Launching staff fulfillment interface." });
    router.push(path);
  };
                        ...
                        return (
                          <div key={mode} className={cn("flex items-center justify-between p-3 rounded-xl border-2 transition-all", isActive ? "bg-white border-primary/20 shadow-sm" : "bg-slate-50 border-slate-100 opacity-60")}>
                            ...
                            <div className="flex items-center gap-3">
                              {isActive && (
                                <button onClick={() => handleImpersonate(mode)} className="p-1.5 rounded-lg hover:bg-slate-100 text-slate-400 hover:text-primary transition-all" title="Impersonate Staff">
                                  <UserCircle className="h-4 w-4" />
                                </button>
                              )}
                              <Switch checked={isActive} onCheckedChange={() => handleToggleMode(mode, !!isActive)} onPointerDown={(e) => e.stopPropagation()} className="data-[state=checked]:bg-primary" />
                            </div>
                          </div>
                        );
                      })}
                ...
                <div className="space-y-12 animate-in fade-in duration-500">
                  <div className="space-y-6"><div className="flex items-center gap-3"><div className="bg-primary/10 p-2 rounded-xl text-primary"><Zap className="h-5 w-5" /></div><h3 className="font-headline font-black text-lg text-[#213147] uppercase">Channel Status</h3></div><div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-3">
                    {['Beverage Cart', 'Clubhouse', 'Lane Delivery', 'Take Out'].map(mode => {
                      ...
                      if (!seller?.menuTypes?.includes(mode)) return null;
                      return (
                        <Card key={mode} className={cn("border-2 transition-all cursor-pointer group flex flex-col", isActive ? "border-primary bg-primary/5 shadow-md" : "border-slate-100 hover:border-slate-200")}>
                          <CardContent className="p-3 flex flex-col items-center text-center gap-2 flex-1" onClick={() => handleToggleMode(mode, !!isActive)}>
                          ...
                          {isActive && (
                            <div className="p-2 border-t bg-slate-50/50 flex justify-center">
                              <Button 
                                ...
                                className="w-full h-8 text-[8px] font-black uppercase tracking-widest gap-1.5 text-slate-500 hover:text-primary hover:bg-primary/5"
                              >
                                <UserCircle className="h-3 w-3" /> Impersonate Staff
                              </Button>
                            </div>
                          )}
                        </Card>
